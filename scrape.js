...
// Your HTML content
const html = `
<select className=' bg-transparent outline-none border-2 border-black rounded p-2'>
...
app.get('/', (req, res) => {
  ...
  // Iterate through each option and extract the value and content
  options.each((index, element) => {
    const _id = $(element).val();
    const sector = $(element).text().replace(/\s+/g, ' ').trim();

    // Push the data to the array
    dataArray.push({ _id, sector });
  });
  ...
  res.send('Data has been written to output.json');
});
